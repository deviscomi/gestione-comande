...
export default function MenuKitchen() {
  ...
  const { data: categories } = useQuery({
    queryKey: ['categories', 'cucina'],
    queryFn: () => menuApi.getCategories({ department: 'cucina' }).then(r => r.data.data),
  })

  const { data: dishes } = useQuery({
    queryKey: ['dishes', 'cucina'],
    queryFn: () => menuApi.getDishes({ department: 'cucina' }).then(r => r.data.data),
  })

  const { data: ingredients } = useQuery({
    queryKey: ['ingredients', 'cucina'],
    queryFn: () => menuApi.getIngredients({ department: 'cucina' }).then(r => r.data.data),
  })

  const { data: variantGroups } = useQuery({
    queryKey: ['dish-variant-groups', 'cucina'],
    queryFn: () => menuApi.getDishVariantGroups({ department: 'cucina' }).then(r => r.data.data),
  })
  ...
  const toggleVariantGroupMut = useMutation({
    ...
    onSuccess: () => qc.invalidateQueries({ queryKey: ['dish-variant-groups', 'cucina'] }),
  })
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <h1 style={{ fontSize: 20, fontWeight: 700 }}>Menu Cucina</h1>
        {tab === 'dishes' && (
      ...
      {/* ══════ INGREDIENTI ══════ */}
      {tab === 'ingredients' && <IngredientManager department="cucina" />}
      ...
      {editVariantGroup !== null && (
        <DishVariantGroupForm
          ...
          onSave={() => { setEditVariantGroup(null); qc.invalidateQueries({ queryKey: ['dish-variant-groups', 'cucina'] }) }}
        />
      )}
    </div>
  )
