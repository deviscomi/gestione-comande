...
<table>
  <thead>
    <tr>
      <th>Cameriere</th>
      <th class="num">Tavoli serviti</th>
  ...
  <tbody>
    @forelse($data['rows'] ?? [] as $row)
      <tr>
        <td>{{ trim(($row['user']['name'] ?? '') . ' ' . ($row['user']['surname'] ?? '')) ?: ('Utente #' . ($row['user_id'] ?? '?')) }}</td>
        <td class="num">{{ $row['tables_served'] ?? 0 }}</td>
        <td class="num">{{ $row['total_covers'] ?? 0 }}</td>
