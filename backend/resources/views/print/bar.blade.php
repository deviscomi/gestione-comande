...
<p style="text-align:center; font-weight:bold;">* BAR *</p>
...
@php
  $barDepartments = \App\Services\PrintDispatcher::BAR_DEPARTMENTS;
  $items = $job->orderSend->items
      ->sortBy('sort_order')
      ->filter(fn($i) => $i->item_type === 'wine' || in_array($i->dish?->category?->department, $barDepartments));
  $grouped = $items->groupBy(fn($i) => $i->item_type === 'wine' ? ($i->wine?->category?->name ?? 'Vini') : ($i->dish?->category?->name ?? 'Altro'));
@endphp

@foreach($grouped as $catName => $catItems)
<div class="cat">--- {{ strtoupper($catName) }} ---</div>
@foreach($catItems as $item)
<div class="item">x{{ $item->quantity }} {{ $item->item_type === 'wine' ? $item->wine?->name : $item->dish?->name }}</div>
